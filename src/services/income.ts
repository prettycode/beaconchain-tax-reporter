...
export async function getValidatorWithdrawals(authKey: string, startingEpoch: number, validatorIndices: Array<number>, endEpoch: number): Promise<Array<ValidatorIncome>> {
    ...
    for (
        ...
        for (let withdrawal of withdrawals) {
            ...
            const withdrawalIncome: ValidatorIncome = {
                ...
                location: `Withdrawal ${withdrawal.withdrawalindex}, epoch ${withdrawal.epoch}, slot ${withdrawal.slot}, block ${withdrawalSlot.exec_block_number}`,
            ...
            console.log(`• ${withdrawalIncome.amountEth} ETH withdrawal payout for ${withdrawalIncome.validatorIndex} in epoch ${withdrawalIncome.extended?.epoch} ${withdrawalIncome.timestamp}`);
        }
    }
...
export async function getValidatorExecutions(authKey: string, validatorIndices: Array<number>): Promise<Array<ValidatorIncome>> {
    ...
    executionHistory.forEach(execution => console.log(`• ${execution.amountEth} ETH producer payout for ${execution.validatorIndex} for block ${execution.extended?.blockNumber} ${execution.timestamp}`));
